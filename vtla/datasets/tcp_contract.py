...
def validate_tcp_contract(contract: dict | None, *, offsets=None, robot_type=None,
                          warn_offset_mismatch: bool = False) -> None:
    ...
    if offsets is not None and list(offsets) != stored:
        ...
        if not warn_offset_mismatch:
            raise ValueError(f"TCP statistics offsets {stored} do not match training offsets {requested}; "
                             "rebuild relative statistics with the matching horizon/action-gap.")
        warnings.warn(
            f"TCP statistics offsets {stored} do not match policy offsets {requested}; "
            "normalization may be less accurate.",
