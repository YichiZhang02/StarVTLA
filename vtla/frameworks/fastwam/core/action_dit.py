...
OFFICIAL_WAN22_DIT_MODEL_HASH = "1f5ab7703c6fc803fdded85ff040c316"
...
class ActionDiT(nn.Module):
    ...
    def from_pretrained(
        ...
        if source:
            logger.info("ActionDiT backbone source: %s", source)
        if p.name.startswith("InterpolatedDiT_from_official_Wan2.2"):
            if source.get("repo_id") != OFFICIAL_WAN22_REPO_ID:
                raise ValueError(
                    f"Official ActionDiT payload must come from {OFFICIAL_WAN22_REPO_ID!r}, "
                    f"got {source.get('repo_id')!r}."
                )
            if not source.get("commit"):
                raise ValueError("Official ActionDiT payload is missing its source commit.")
            if source.get("video_dit_model_hash") != OFFICIAL_WAN22_DIT_MODEL_HASH:
                raise ValueError(
                    "Official ActionDiT payload has an unexpected Wan2.2 Video DiT model hash: "
                    f"{source.get('video_dit_model_hash')!r}."
                )
