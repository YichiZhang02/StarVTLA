...
class FastWAM(torch.nn.Module):
    ...
    def from_wan22_pretrained(
        ...
        video_dit_pretrained_path: str | list[str] | None = None,
        ...
        components = load_wan22_ti2v_5b_components(
            ...
            dit_path=video_dit_pretrained_path,
            vae_path=vae_pretrained_path,
            text_encoder_path=text_encoder_pretrained_path,
            tokenizer_path=tokenizer_pretrained_path,
        )
    ...
    def _build_expert_contexts(
        ...
        context: torch.Tensor,
        context_mask: torch.Tensor,
        tactile_context: Optional[torch.Tensor],
        tactile_context_mask: Optional[torch.Tensor],
        tactile_insert_location: str,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        video_context, video_mask = context, context_mask
        action_context, action_mask = context, context_mask
        if tactile_context is None and tactile_context_mask is None:
            return video_context, video_mask, action_context, action_mask
        if tactile_context is None or tactile_context_mask is None:
            raise ValueError("`tactile_context` and `tactile_context_mask` must be provided together.")
        if tactile_insert_location not in ("encoder", "decoder"):
            raise ValueError(
                "FastWAM tactile_insert_location must be 'encoder' or 'decoder', "
                f"got {tactile_insert_location!r}."
            )
        tactile_context = tactile_context.to(device=self.device, dtype=context.dtype, non_blocking=True)
        tactile_context_mask = tactile_context_mask.to(
            device=self.device, dtype=torch.bool, non_blocking=True
        )
        if tactile_context.ndim != 3 or tactile_context_mask.ndim != 2:
            raise ValueError(
                "FastWAM tactile context must be [B,N,D] with mask [B,N], got "
                f"{tuple(tactile_context.shape)} and {tuple(tactile_context_mask.shape)}."
            )
        if tactile_context.shape[0] != context.shape[0] or tactile_context.shape[2] != self.text_dim:
            raise ValueError(
                "FastWAM tactile context batch/feature shape mismatch: "
                f"{tuple(tactile_context.shape)} vs text context {tuple(context.shape)}."
            )
        if tactile_context_mask.shape != tactile_context.shape[:2]:
            raise ValueError("FastWAM tactile context mask shape does not match tactile tokens.")
        # Both modes are conditioning-only: encoder routes to Video DiT context,
        # decoder preserves the existing Action DiT context route.
        if tactile_insert_location == "encoder":
            video_context = torch.cat([video_context, tactile_context], dim=1)
            video_mask = torch.cat([video_mask, tactile_context_mask], dim=1)
        else:
            action_context = torch.cat([action_context, tactile_context], dim=1)
            action_mask = torch.cat([action_mask, tactile_context_mask], dim=1)
        return video_context, video_mask, action_context, action_mask
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        tactile_context = sample.get("tactile_context")
        tactile_context_mask = sample.get("tactile_context_mask")
        tactile_insert_location = str(sample.get("tactile_insert_location", "decoder"))
        if video.ndim != 5:
        ...
        video_context, video_context_mask, action_context, action_context_mask = self._build_expert_contexts(
            context=context,
            context_mask=context_mask,
            tactile_context=tactile_context,
            tactile_context_mask=tactile_context_mask,
            tactile_insert_location=tactile_insert_location,
        )
    ...
    def infer_joint(
        ...
        tactile_context: Optional[torch.Tensor] = None,
        tactile_context_mask: Optional[torch.Tensor] = None,
        tactile_insert_location: str = "decoder",
        ...
        if test_action_with_infer_action:
            ...
            action_only_out = self.infer_action(
                ...
                tactile_context=tactile_context.clone() if tactile_context is not None else None,
                tactile_context_mask=(
                    tactile_context_mask.clone() if tactile_context_mask is not None else None
                ),
                tactile_insert_location=tactile_insert_location,
