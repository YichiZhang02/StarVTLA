...
from vtla.engine.optim import AdamConfig, DiffuserSchedulerConfig
...
@dataclass
class DiffusionConfig(SensorRoutingMixin, PreTrainedConfig):
    ...
    # All routed cameras are bilinearly resized to this common (H, W) before the (single,
    # shared) vision backbone. This is what lets cameras with different native resolutions
    # (e.g. top 896, wrist 480, finger 240x320) be stacked and trained together.
    # Set to None to restore the original strict "all images must already match" behaviour.
    resize_imgs_to: tuple[int, int] | None = (224, 224)
    ...
    def validate_features(self) -> None:
        ...
        # If `resize_imgs_to` is set, unify every camera's declared shape to it now, so the
        # downstream crop check, the equality check and the encoder's dry-run all see the
        # common size. The actual tensors are resized to match in DiffusionPolicy.forward.
        if self.resize_imgs_to is not None:
            target_h, target_w = self.resize_imgs_to
            for image_ft in self.image_features.values():
                image_ft.shape = (image_ft.shape[0], target_h, target_w)
            ...
            for key, image_ft in self.image_features.items():
                if image_ft.shape != first_image_ft.shape:
                    raise ValueError(
                        ...
                        f"Set `resize_imgs_to=(H, W)` to auto-resize all cameras to a common size."
                    )
