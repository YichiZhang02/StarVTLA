# Adapted from N0-VTLA, NeoteAI Team / Fudan TEAI Team.
# Original tactile components: CC BY-SA 4.0; see LICENSE and NOTICE.
...
class FrozenDINOv2TactileEncoder(nn.Module):
    ...
    def __init__(self, llm_dim: int, pool_grid: int = 3, backbone_config: dict | None = None):
        super().__init__()
        self.pool_grid = pool_grid
        # facebook/dinov2-base stores a 37x37 positional grid (518 / 14),
        # unlike Dinov2Config's generic 224 default. DINO interpolates this
        # pretrained grid for our 224 tactile inputs; retain all saved weights.
        config = {"image_size": 518, **(backbone_config or {})}
        self.backbone = Dinov2Model(Dinov2Config(**config))
        for p in self.backbone.parameters():
            p.requires_grad = False
        self.backbone.eval()
        self.tactile_proj = nn.Linear(self.backbone.config.hidden_size, llm_dim)
    ...
    def encode_backbone(self, imgs: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            hs = self.backbone(pixel_values=imgs.to(next(self.backbone.parameters()).dtype)).last_hidden_state
        cls = hs[:, :1]
