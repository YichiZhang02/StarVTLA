...
def _resolve_local_dcp_checkpoint_path(checkpoint_dir: str):
    """Resolve a local job/checkpoint directory to the actual DCP model path (iter_XXXXX/model)."""
    checkpoint_dir = checkpoint_dir.rstrip("/")
    if checkpoint_dir.endswith("/model"):
        return checkpoint_dir
    if "/iter_" in checkpoint_dir and os.path.isdir(checkpoint_dir):
        model_dir = os.path.join(checkpoint_dir, "model")
        if os.path.isdir(model_dir):
            return model_dir
        return checkpoint_dir
    # Job output root: .../job_name -> checkpoints/latest
    latest_txt = os.path.join(checkpoint_dir, "checkpoints", "latest_checkpoint.txt")
    if os.path.isfile(latest_txt):
        with open(latest_txt) as f:
            iter_name = f.read().strip()
        resolved = os.path.join(checkpoint_dir, "checkpoints", iter_name, "model")
        if os.path.isdir(resolved):
            return resolved
        return os.path.join(checkpoint_dir, "checkpoints", iter_name)
    # Parent of job folder (e.g. .../cosmos_v2_finetune with child cosmos_predict2_2b_...): try single subdir
    if os.path.isdir(checkpoint_dir):
        subdirs = [d for d in os.listdir(checkpoint_dir) if os.path.isdir(os.path.join(checkpoint_dir, d)) and not d.startswith(".")]
        if len(subdirs) == 1:
            job_dir = os.path.join(checkpoint_dir, subdirs[0])
            latest_txt = os.path.join(job_dir, "checkpoints", "latest_checkpoint.txt")
            if os.path.isfile(latest_txt):
                with open(latest_txt) as f:
                    iter_name = f.read().strip()
                resolved = os.path.join(job_dir, "checkpoints", iter_name, "model")
                if os.path.isdir(resolved):
                    return resolved
                return os.path.join(job_dir, "checkpoints", iter_name)
...
def load_model_state_dict_from_checkpoint(
    ...
    if s3_checkpoint_dir.startswith("s3:"):
        ...
        cur_key_ckpt_full_path = s3_checkpoint_dir
        if checkpoint_format == "dcp" and os.path.isdir(cur_key_ckpt_full_path):
            cur_key_ckpt_full_path = _resolve_local_dcp_checkpoint_path(cur_key_ckpt_full_path)
            log.info(f"Resolved local DCP checkpoint path to {cur_key_ckpt_full_path}")
    ...
    # For local DCP (directory), use DistributedCheckpointer instead of easy_io.load
    use_dcp_local = (
        load_from_local
        and checkpoint_format == "dcp"
        and not s3_checkpoint_dir.startswith("s3:")
        and os.path.isdir(local_s3_ckpt_fp)
    )

    if load_from_local and not use_dcp_local:
        # Load on rank0 only and broadcast (.pt file)
        ...
        # Local DCP checkpoint (directory): use DistributedCheckpointer with filesystem reader
        log.info(f"Loading model from local DCP checkpoint {cur_key_ckpt_full_path}")
