...
from cosmos_policy._src.imaginaire.utils.context_parallel import broadcast_split_tensor, find_split
from cosmos_policy._src.predict2.conditioner import (
    ...
    ScalarOptimalityToken,
)
from cosmos_policy._src.predict2.models.video2world_wan2pt1_model import WAN2PT1_I2V_COND_LATENT_KEY
from cosmos_policy._src.predict2.networks.clip import Wan2pt1CLIPEmb
...
_SHARED_CONFIG = dict(
    ...
    # CFGRL-style optimality token:
    # - During conditioned sampling, override dropout to 0 to include the token.
    # - During unconditional sampling (CFG uncond branch), default dropout_rate=1.0 drops to null_bin.
    optimality_token=L(ScalarOptimalityToken)(
        input_key=["t5_text_embeddings", "optimality"],
        dropout_rate=1.0,
        num_bins=2,
        null_bin=-1,
        token_scale=1.0,
