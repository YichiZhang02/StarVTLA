...
def build_tactile_outer_vectors_1d(
    T: int,
    H: int,
    W: int,
    tactile_latent_t_indices: Sequence[int],
    device: torch.device,
    dtype: torch.dtype,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    For flattened token order (t,h,w) lexicographic, M=1 on tactile latent time indices.
    Returns a_i = 1 - M_i, b_j = M_j so bias_ij = gamma * a_i * b_j is non-tactile query -> tactile key only.
    """
    S = T * H * W
    M = torch.zeros(S, device=device, dtype=dtype)
    stride = H * W
    for t in tactile_latent_t_indices:
        if 0 <= t < T:
            M[t * stride : (t + 1) * stride] = 1.0
    a = 1.0 - M
    b = M
    return a, b
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            and tactile_outer_b_S is not None
            and tactile_gamma_B is not None
        ):
            return self_attention_with_tactile_outer_bias_chunked(
                q,
                k,
                v,
                tactile_outer_a_S,
...
class MiniTrainDIT(WeightTrainingStat):
    ...
    def __init__(
        ...
        use_wan_fp32_strategy: bool = False,
        # Tactile self-attn: additive logits bias gamma * (1-M_i) * M_j on keys (non-tactile query -> tactile key)
    ...
    def _tactile_self_attn_block_kw(
        self,
        x_B_T_H_W_D: torch.Tensor,
        tactile_self_attn_gate_B: Optional[torch.Tensor],
    ) -> dict:
        if (
            not self.use_tactile_self_attn_bias
            or tactile_self_attn_gate_B is None
    ...
    def forward(
        ...
        # Optional debug capture: mapped timestep embedding after Timesteps + TimestepEmbedding + RMSNorm.
        if getattr(self, "_capture_debug_mapped_t_embedding", False):
            if not hasattr(self, "_debug_mapped_t_embedding_history"):
                self._debug_mapped_t_embedding_history = []
            self._debug_mapped_t_embedding_history.append(t_embedding_B_T_D.detach().to(torch.float32).cpu())
        ...
        tactile_kw = self._tactile_self_attn_block_kw(x_B_T_H_W_D, tactile_self_attn_gate_B)
