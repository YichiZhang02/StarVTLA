...
class ScalarOptimalityToken(AbstractEmbModel):
    """
    Deterministic, non-trainable optimality token for CFGRL-style guidance.

    This embedder appends a single token (shape Bx1xD) into `crossattn_emb` by
    converting a scalar `optimality` label into a broadcast vector of the same
    hidden size D as the provided reference embedding (e.g. T5 embeddings).

    Design constraints (important for this codebase):
    - Conditioner is expected to have *no trainable parameters*.
    - We avoid introducing new condition keys that would require changing
      `net.forward(...)` signatures. Instead we reuse `crossattn_emb` concat.
    """
    ...
    def random_dropout_input(
        ...
        # We only want to dropout the optimality label, not the reference embedding.
        if key is None or key != "optimality":
            return in_tensor

        dropout_rate = dropout_rate if dropout_rate is not None else self.dropout_rate
        if dropout_rate <= 1e-8:
            return in_tensor
        if dropout_rate >= 1.0 - 1e-8:
            return torch.full_like(in_tensor, fill_value=self.null_bin)

        B = in_tensor.shape[0]
        keep_mask = torch.bernoulli((1.0 - dropout_rate) * torch.ones(B, device=in_tensor.device)).to(
            dtype=in_tensor.dtype
        )
        keep_mask = keep_mask.view(B, *[1] * (in_tensor.dim() - 1))
        dropped = torch.full_like(in_tensor, fill_value=self.null_bin)
        return keep_mask * in_tensor + (1.0 - keep_mask) * dropped

    def forward(self, ref_emb: torch.Tensor, optimality: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Args:
            ref_emb: (B, L, D) cross-attention embeddings (e.g. T5).
            optimality: (B,) or (B,1) integer-like bins in [-1, num_bins-1].
                - null_bin (-1) indicates unconditional (o = ∅).
        Returns:
            dict with key `crossattn_emb`: (B, 1, D) token to be concatenated by GeneralConditioner.
        """
        if optimality.ndim == 2 and optimality.shape[1] == 1:
            optimality = optimality[:, 0]
        if optimality.ndim != 1:
            raise ValueError(f"optimality must be shape (B,) or (B,1), got {tuple(optimality.shape)}")

        B, _, D = ref_emb.shape
        opt_f = optimality.to(device=ref_emb.device).float()

        # Map bins -> [-1, 1]. null_bin maps to 0.
        # For num_bins=2: {0,1} -> {-1,+1}
        # For K bins: k -> 2*k/(K-1)-1
        is_null = (optimality == self.null_bin).to(device=ref_emb.device)
        opt_scaled = (2.0 * opt_f / float(self.num_bins - 1)) - 1.0
        opt_scaled = torch.where(is_null, torch.zeros_like(opt_scaled), opt_scaled)

        token = opt_scaled.view(B, 1, 1) * torch.ones((B, 1, D), device=ref_emb.device, dtype=ref_emb.dtype)
        token = token * self.token_scale
        return {"crossattn_emb": token}

    def details(self) -> str:
        return (
            "Output key: [crossattn_emb]\n\tDeterministic scalar->token optimality conditioning for CFG-RL"
            f"\n\tnum_bins: {self.num_bins}, null_bin: {self.null_bin}, token_scale: {self.token_scale}"
        )
