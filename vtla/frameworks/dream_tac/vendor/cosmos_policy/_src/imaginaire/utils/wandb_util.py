...
from cosmos_policy._src.imaginaire.utils import distributed, log, object_store
...
def init_wandb(config: Config, model: ImaginaireModel) -> None:
    ...
    # Use fixed wandb_id from config if set (so repeated runs log to the same run).
    wandb_id = getattr(config_job, "wandb_id", None) or None
    if wandb_id:
        log.info(f"Using fixed wandb ID from config: {wandb_id}")
    else:
