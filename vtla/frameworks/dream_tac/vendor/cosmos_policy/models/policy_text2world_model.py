...
class CosmosPolicyDiffusionModel(BaseDiffusionModel):
    ...
    def training_step(
        ...
        output_batch, kendall_loss, _, _ = self.compute_loss_with_epsilon_and_sigma(
            ...
            future_tactile_left_indices=(
                data_batch["future_tactile_left_latent_idx"] if "future_tactile_left_latent_idx" in data_batch else None
            ),
    ...
    def compute_loss_with_epsilon_and_sigma(
        ...
        # Value: only overwrite when we have a dedicated value frame (value_indices != -1).
        # When value_latent_idx=-1 (e.g. Franka return_value_function_returns=False), do NOT write:
        # otherwise we would write to x0[..., -1, ...] = last frame and overwrite future primary image with -100.
        ...
        if self.config.mask_loss_for_action_future_state_prediction:
            ...
            if torch.any(world_idx_B):
                ...
                if future_tactile_left_indices is not None and torch.all(future_tactile_left_indices != -1):
        ...
        # Get losses for future tactile left prediction
        if future_tactile_left_indices is not None and torch.all(future_tactile_left_indices != -1):
            future_tactile_left_diff = (
                x0_B_C_T_H_W[batch_indices, :, future_tactile_left_indices, :, :]
                - model_pred.x0[batch_indices, :, future_tactile_left_indices, :, :]
            )
            future_tactile_left_diff_demo = future_tactile_left_diff[rollout_data_mask == 0]
            demo_sample_future_tactile_left_mse_loss = (future_tactile_left_diff_demo**2).mean()
            demo_sample_future_tactile_left_l1_loss = torch.abs(future_tactile_left_diff_demo).mean()
        else:
            demo_sample_future_tactile_left_mse_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            demo_sample_future_tactile_left_l1_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)

        # Get losses for future tactile right prediction
        if future_tactile_right_indices is not None and torch.all(future_tactile_right_indices != -1):
            future_tactile_right_diff = (
                x0_B_C_T_H_W[batch_indices, :, future_tactile_right_indices, :, :]
                - model_pred.x0[batch_indices, :, future_tactile_right_indices, :, :]
            )
            future_tactile_right_diff_demo = future_tactile_right_diff[rollout_data_mask == 0]
            demo_sample_future_tactile_right_mse_loss = (future_tactile_right_diff_demo**2).mean()
            demo_sample_future_tactile_right_l1_loss = torch.abs(future_tactile_right_diff_demo).mean()
        else:
            demo_sample_future_tactile_right_mse_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
            demo_sample_future_tactile_right_l1_loss = torch.tensor(float("nan"), device=x0_B_C_T_H_W.device)
        ...
        # Get losses for value function prediction (only when we have a value frame, value_indices != -1)
        if torch.all(value_indices != -1):
            value_diff = (
                x0_B_C_T_H_W[batch_indices, :, value_indices, :, :]
        ...
        output_batch = {
            ...
            "demo_sample_future_tactile_left_mse_loss": demo_sample_future_tactile_left_mse_loss,
            "demo_sample_future_tactile_left_l1_loss": demo_sample_future_tactile_left_l1_loss,
            "demo_sample_future_tactile_right_mse_loss": demo_sample_future_tactile_right_mse_loss,
            "demo_sample_future_tactile_right_l1_loss": demo_sample_future_tactile_right_l1_loss,
