...
logger_cosmos_utils = logging.getLogger(__name__)
...
def get_latent_indices_from_model_config(model):
    ...
    if state_t == 9 and min_conditional_frames == 4:
        ...
        return (1, 4, 6, 9)
    elif state_t == 12 and min_conditional_frames == 6:
        # Franka with tactile (proprio, wrist, primary, tactile_left, tactile_right | action | future x5)
        return (1, 6, 7, 12)
    else:
        raise ValueError(
...
def get_t5_embedding_from_cache(task_label: str) -> torch.Tensor:
    """
    Get T5 embedding of language instruction from cache (pkl only).

    Does not compute new embeddings; reuses the same pkl. If the instruction is not
    in cache, uses the first available embedding from the cache as fallback.
    ...
    if task_label in t5_text_embeddings_cache:
        return t5_text_embeddings_cache[task_label]
    # Reuse pkl only: do not compute. Use first embedding from cache as fallback.
    if t5_text_embeddings_cache:
        fallback = next(iter(t5_text_embeddings_cache.values()))
        logger_cosmos_utils.debug(
            "Instruction '%s' not in T5 cache; reusing first embedding from pkl (no new computation).",
            task_label,
        )
        return fallback
    raise KeyError(
        f"T5 embedding for instruction '{task_label}' not in cache and cache is empty. "
        "Load a pkl with precomputed embeddings (e.g. t5_text_embeddings_path) and do not compute new ones."
    )
...
def prepare_images_for_model(images: List[np.ndarray], cfg, flip_images: bool = False) -> List[Image.Image]:
    ...
    """
    # Resize any images with mismatched shapes before stacking
    if len(images) > 1:
        shapes = [img.shape[:2] for img in images]
        if len(set(shapes)) > 1:
            images = [
                np.array(Image.fromarray(img).resize((COSMOS_IMAGE_SIZE, COSMOS_IMAGE_SIZE), Image.BICUBIC))
                if img.shape[:2] != (COSMOS_IMAGE_SIZE, COSMOS_IMAGE_SIZE) else img
                for img in images
            ]
...
def get_future_images_from_generated_samples(
    ...
    future_tactile_left_latent_idx: int = -1,
    future_tactile_right_latent_idx: int = -1,
    ...
    if future_tactile_left_latent_idx >= 0:
        ftl_raw_idx = (future_tactile_left_latent_idx - 1) * temporal_compression_factor + 1
        future_image_predictions["future_tactile_left"] = generated_images[:, ftl_raw_idx]
    if future_tactile_right_latent_idx >= 0:
        ftr_raw_idx = (future_tactile_right_latent_idx - 1) * temporal_compression_factor + 1
        future_image_predictions["future_tactile_right"] = generated_images[:, ftr_raw_idx]
...
def get_action(
    ...
    with torch.inference_mode():
        ...
        if cfg.suite == "libero":
            ...
            WRIST_IMAGE_IDX = 0
            IMAGE_IDX = 1
        elif cfg.suite == "franka":
            # Franka: cam_high -> wrist_image, cam_front -> primary_image (same layout as LIBERO)
            all_camera_images = [
                obs["wrist_image"],
                obs["primary_image"],
            ]
            WRIST_IMAGE_IDX = 0
            IMAGE_IDX = 1
            TACTILE_LEFT_IDX, TACTILE_RIGHT_IDX = -1, -1
            if getattr(cfg, "use_tactile", False) and "tactile_left_image" in obs:
                TACTILE_LEFT_IDX = len(all_camera_images)
                all_camera_images.append(obs["tactile_left_image"])
                TACTILE_RIGHT_IDX = len(all_camera_images)
                all_camera_images.append(obs["tactile_right_image"])
        ...
        # Add current tactile images (Franka tactile, state_t=12)
        future_tactile_left_latent_idx = -1
        future_tactile_right_latent_idx = -1
        if getattr(cfg, "use_tactile", False) and TACTILE_LEFT_IDX >= 0:
            tactile_left_img = all_camera_images[TACTILE_LEFT_IDX]
            tactile_left_duplicated = duplicate_array(tactile_left_img, total_num_copies=COSMOS_TEMPORAL_COMPRESSION_FACTOR)
            image_sequence.append(tactile_left_duplicated)
            current_tactile_left_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            tactile_right_img = all_camera_images[TACTILE_RIGHT_IDX]
            tactile_right_duplicated = duplicate_array(tactile_right_img, total_num_copies=COSMOS_TEMPORAL_COMPRESSION_FACTOR)
            image_sequence.append(tactile_right_duplicated)
            current_tactile_right_latent_idx = current_sequence_idx
            current_sequence_idx += 1
        ...
        # Add placeholders for the future tactile images
        if getattr(cfg, "use_tactile", False) and TACTILE_LEFT_IDX >= 0:
            image_sequence.append(tactile_left_duplicated.copy())
            future_tactile_left_latent_idx = current_sequence_idx
            current_sequence_idx += 1
            image_sequence.append(tactile_right_duplicated.copy())
            future_tactile_right_latent_idx = current_sequence_idx
            current_sequence_idx += 1

        # Add placeholder for the value only when the model was trained with value (state_t=9 or 11).
        # When state_t=8 (e.g. Franka trained without value), do not add value slot so sequence length matches.
        use_value_in_sequence = model.config.state_t in (9, 11)
        if use_value_in_sequence:
            image_sequence.append(blank_image_duplicated.copy())
        ...
        # Ensure video has exactly the number of frames the model expects (fixes 8 vs 20 dim mismatch)
        expected_num_frames = model.tokenizer.get_pixel_num_frames(model.config.state_t)
        current_num_frames = raw_image_sequence.shape[2]
        if current_num_frames != expected_num_frames:
            if current_num_frames < expected_num_frames:
                # Pad with last frame
                pad_frames = expected_num_frames - current_num_frames
                pad = np.tile(raw_image_sequence[:, :, -1:], (1, 1, pad_frames, 1, 1))
                raw_image_sequence = np.concatenate([raw_image_sequence, pad], axis=2)
            else:
                # Truncate
                raw_image_sequence = raw_image_sequence[:, :, :expected_num_frames]
        ...
        # Open-loop hard residual cache: reset DiT sub-step counter once per diffusion sampling.
        if getattr(model, "_openloop_hard_residual_cache", False):
            from cosmos_policy.experiments.robot.openloop_hard_residual_cache import reset_openloop_denoise_counter

            reset_openloop_denoise_counter(model)
        # Optional debug capture of mapped timestep embeddings from the DiT net:
        # Timesteps -> TimestepEmbedding -> RMSNorm (minimal_v4_dit).
        if hasattr(model, "net"):
            setattr(model.net, "_capture_debug_mapped_t_embedding", True)
            setattr(model.net, "_debug_mapped_t_embedding_history", [])
        ...
        # Extract the action latent frame from the generated sample.
        action_latent = (
            generated_latent_with_action[:, :, action_latent_idx, :, :].to(torch.float32).detach().cpu().numpy()
        )  # (B, C, H, W)

        # Collect mapped timestep embeddings captured inside minimal_v4_dit forward.
        # Shape per item: (B, T_latent, D_model)
        mapped_timestep_embeddings = []
        if hasattr(model, "net") and hasattr(model.net, "_debug_mapped_t_embedding_history"):
            mapped_timestep_embeddings = [
                x.detach().cpu().numpy() if torch.is_tensor(x) else np.asarray(x)
                for x in getattr(model.net, "_debug_mapped_t_embedding_history", [])
            ]
            setattr(model.net, "_capture_debug_mapped_t_embedding", False)
            setattr(model.net, "_debug_mapped_t_embedding_history", [])
        ...
        if generate_future_state_and_value_in_parallel:
            ...
            elif cfg.suite == "franka" and model.config.state_t == 12:
                INDICES_TO_REPLACE = [
                    0,
                    1,
                    6,
                    7,
                ]  # Franka tactile (state_t=12): 0=blank, 1=proprio, 2=wrist, 3=primary, 4=tactile_left, 5=tactile_right, 6=action, 7=future_proprio
            elif cfg.suite == "franka":
                INDICES_TO_REPLACE = [
                    0,
                    1,
                    4,
                    5,
                ]  # Franka without tactile (state_t=8): same as LIBERO
            elif cfg.suite == "robocasa":
                INDICES_TO_REPLACE = [
                    0,
                    1,
            ...
            future_image_predictions = get_future_images_from_generated_samples(
                ...
                future_tactile_left_latent_idx=future_tactile_left_latent_idx if getattr(cfg, "use_tactile", False) else -1,
                future_tactile_right_latent_idx=future_tactile_right_latent_idx if getattr(cfg, "use_tactile", False) else -1,
                temporal_compression_factor=COSMOS_TEMPORAL_COMPRESSION_FACTOR,
            )
            # Get value predictions from the generated sample (only when model was trained with value)
            if value_latent_idx >= 0:
                value_indices = torch.full((batch_size,), value_latent_idx, dtype=torch.int64, device=generated_latent_with_action.device)
                ...
                value_prediction = torch.zeros(batch_size, device=generated_latent_with_action.device, dtype=torch.bfloat16)
        ...
        if batch_size > 1:
            ...
            action_latent = [action_latent[i] for i in range(batch_size)]
            if generate_future_state_and_value_in_parallel:
        ...
        return_dict = dict(
            ...
            mapped_timestep_embeddings=mapped_timestep_embeddings,
