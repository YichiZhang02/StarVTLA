...
from vtla.engine.utils.constants import ACTION, OBS_ENV_STATE, OBS_IMAGES, OBS_STATE
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        # Tactile-encode branch (tactile_mode="encode"). One token per tactile key,
        # projected into the encoder token space, inserted either on the encoder side
        # (joins the observation tokens) or the decoder side (extra cross-attn memory).
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Tactile tokens on the decoder side: appended to the cross-attention memory
