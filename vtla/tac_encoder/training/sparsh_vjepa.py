...
from vtla.tac_encoder.models.checkpoint import read_state_dict
from vtla.tac_encoder.models.sparsh_vjepa import (
    _SinusoidalPositionEmbedding,
    _SparshEncoder,
)

from .base import StepOutput, TrainingRecipe, WarmupCosineScheduler
...
def _load_full_pretrained(model: SparshVJEPATrainingModel, path: str) -> dict:
    if not path:
        raise ValueError("Sparsh V-JEPA training requires a full pretrained checkpoint")
    ...
    if not (has_context and has_target and has_predictor):
        raise ValueError(
            "Sparsh V-JEPA training requires a full reference checkpoint containing "
            "context_encoder, target_encoder, and predictor. The released "
            "vjepa_vitsmall.safetensors contains the encoder only and cannot satisfy "
            "the no-from-scratch requirement."
        )
    ...
    if missing:
        raise ValueError(
            "Full V-JEPA checkpoint does not cover the complete context/target/predictor state: "
            f"{list(missing)[:12]}"
        )
    return {"source": path, "loaded_tensors": len(compatible), "missing_keys": list(missing)}
