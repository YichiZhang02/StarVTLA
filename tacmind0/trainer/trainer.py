...
class DMTrainer(Trainer):
    """Trainer extension for DMTac05 model training."""

    _FSDP_WRAP_ATTR = "_dmtac05_fsdp_wrap"
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        if getattr(target.config, "tactile_config", None) is not None:
            checkpoint = str(Path(resume_from_checkpoint).resolve())
            if getattr(target, "_tacdream_strict_checkpoint", None) != checkpoint:
                raise RuntimeError(
                    "TacDream resume requires strict_policy_load of the exact resume checkpoint"
                )
            # from_pretrained applies backbone key conversions that Trainer's raw
            # state_dict loader bypasses. Optimizer/scheduler/RNG restore separately.
            logger.info("TacDream model already strictly loaded from {}", checkpoint)
            return
        return super()._load_from_checkpoint(resume_from_checkpoint, model=model)

    def _save(self, output_dir=None, state_dict=None):
        super()._save(output_dir, state_dict=state_dict)
        if getattr(self.model.config, "tactile_config", None) is not None:
            import json
            ...
            norm_path = self.exp_config.data_config.norm_stats_path(
                self.exp_config.model_config.chunk_size
            )
            if Path(norm_path).resolve() != (destination / "norm_stats.json").resolve():
                shutil.copyfile(norm_path, destination / "norm_stats.json")

            def serialize(value):
                """Convert config without deepcopying runtime locks/caches."""
                ...
                if value is None or isinstance(value, (str, int, float, bool)):
                    return value
                # Runtime-only objects (for example StreamSessions' RLock) are
                # intentionally represented without copying their state.
                return str(value)

            (destination / "training_config.json").write_text(
                json.dumps(serialize(self.exp_config), indent=2)
            )
    ...
    def _prepare_for_training(self, *args, **kwargs):
        ...
        def _skip_update_fsdp_plugin_peft(model, accelerator):
            logger.info(
                "Skipping Transformers PEFT FSDP auto-wrap update for DMTac05 LoRA."
            )
    ...
    def _configure_fsdp_precision(self) -> None:
        ...
        if fsdp_plugin is not None:
            ...
            logger.info(
                "DMTac05 FSDP mixed precision policy: {}",
                FSDP_MIXED_PRECISION,
            )
    ...
    def _configure_fsdp_auto_wrap(self) -> None:
        ...
        logger.info(
            "DMTac05 FSDP auto-wrap policy: action expert and final {} VLM layers",
            len(modules) - 1,
        )
    ...
    def _link_exp_config(self) -> TrainingArguments:
        ...
        if linked_args["dataloader_num_workers"] == 0:
            linked_args["dataloader_prefetch_factor"] = None
            linked_args["dataloader_persistent_workers"] = False
