...
import tacmind0.data.normalize as normalize
from tacmind0.constants.precision import (
    BF16_MIXED_PRECISION_POLICY,
    FP32_MIXED_PRECISION_POLICY,
    MODEL_DTYPE,
)
from tacmind0.constants.robot import ROBOT_STATE_DESCS, ActionMode, RobotType
from tacmind0.data.augmentations import NoAugmentationPipeline, TrainingTransformPipeline
from tacmind0.data.collator import NormStatsCollator, TrainingCollator
...
from tacmind0.dataset.register import CONVERSATION_DATA
from tacmind0.model.dmtac05.dmtac05_arch import (
    DMTac05Config,
    DMTac05ForConditionalGeneration,
)
from tacmind0.model.dmtac05.dmtac05_lora import (
    DMTac05LoraConfig,
    _is_lora_checkpoint,
    apply_lora_to_dmtac05_model,
    load_dmtac05_model_for_inference,
    unwrap_dmtac05_model,
)
from tacmind0.model.dmtac05.dmtac05_utils import (
    configure_fp32_precision_runtime,
    dmtac05_autocast,
    prepare_action_chunk,
)
from tacmind0.optimizer import MuonAdamW, mark_muon_parameters
from tacmind0.optimizer.muon_adamw import is_default_muon_parameter
from tacmind0.trainer.trainer import DMTrainer, safe_save_model_for_hf_trainer
...
@dataclass
class DMTac05ModelConfig(Config):
    model_name_or_path: str | None = field(default="./checkpoints/DMTac05")
    ...
    lora_config: DMTac05LoraConfig = field(default_factory=DMTac05LoraConfig)
    ...
    def _load_base_checkpoint_model(self) -> DMTac05ForConditionalGeneration:
        config = DMTac05Config.from_pretrained(self.model_name_or_path)
        for attr, value in self._config_overrides().items():
            setattr(config, attr, value)
        return DMTac05ForConditionalGeneration.from_pretrained(
    ...
    def _load_adapter_checkpoint_model(self) -> torch.nn.Module:
        return load_dmtac05_model_for_inference(
            self.model_name_or_path,
    ...
    def _set_precision_policy(self, model: torch.nn.Module) -> None:
        dmtac05_model = unwrap_dmtac05_model(model)
        dmtac05_model.set_precision_policy(self.precision_policy)
    ...
    def _apply_runtime_model_options(
        ...
        dmtac05_model = unwrap_dmtac05_model(model)
        dmtac05_model.set_attention_implementation(
        ...
        dmtac05_model.enable_gradient_checkpointing(
        ...
        if self.freeze_vlm_embedding:
            dmtac05_model.freeze_vlm_embedding()

    def _apply_full_model_runtime_optimizations(
        self, model: DMTac05ForConditionalGeneration
    ) -> None:
        if self.liger_kernel and torch.cuda.is_available():
            model._apply_liger_kernel()
        elif self.liger_kernel:
            logger.info("CUDA unavailable; skipping Liger Kernel for DMTac05 model.")

    def _wrap_new_lora_for_training(
        self, model: DMTac05ForConditionalGeneration
    ) -> torch.nn.Module:
        return apply_lora_to_dmtac05_model(
            model,
            self.lora_config,
...
@dataclass
class DMTac05OptimizerConfig(Config):
...
@dataclass
class DMTac05TrainerConfig(Config):
...
@dataclass
class DMTac05DataConfig(Config):
...
@dataclass
class DMTac05InferenceConfig(Config):
    ...
    def _initialize(
        self,
        model: DMTac05ForConditionalGeneration,
        ...
        if (
            getattr(model.config, "tactile_config", None) is not None
            and self.backend == "fast"
        ):
            raise ValueError(
                "TacDream tactile checkpoints require the default PyTorch backend"
            )
        ...
        if self.backend == "fast":
            from tacmind0.infer.dmtac05_infer import DMTac05FastInferRuntime
            from tacmind0.infer.dmtac05_trt_utils import (
                ensure_dmtac05_fast_inference_engine,
            )

            dmtac05_model = unwrap_dmtac05_model(model)
            num_vision_images = len(self.image_prompts)
            if is_history:
                from tacmind0.infer.dmtac05_trt_utils import MAX_HISTORY_IMAGES

                num_vision_images += MAX_HISTORY_IMAGES
            engine_path = ensure_dmtac05_fast_inference_engine(
            ...
            self.fast_runtime = DMTac05FastInferRuntime(
                dmtac05_model,
            ...
            vlm_model = dmtac05_model.model.vlm.model
        ...
        self.action_normalize = Normalize(
            norm_stats_path=norm_stats_path,
            norm_keys=["action"],
            use_quantiles=True,
            norm_stats_file=self.norm_stats_file,
        )
        self.action_relative = ActionRelative()
    ...
    def _decode_b64_image(
        self, value, field: str, *, require_rgb: bool = False
    ) -> Image.Image:
        ...
        try:
            ...
            if require_rgb and image.mode != "RGB":
                raise ValueError(
                    "Marker image must use RGB mode with three uint8 channels"
                )
    ...
    def _predict(self, data: dict) -> np.ndarray:
        ...
        inference_seed = os.environ.get("DMTac05_INFERENCE_SEED")
        ...
        dmtac05_model = unwrap_dmtac05_model(self.model)
        action_dim = dmtac05_model.model.config.action_dim
        action_mask = torch.zeros(
            1,
            1,
            action_dim,
            device=self.device,
            dtype=dmtac05_model.model.action_in_proj.weight.dtype,
        )
        ...
        inference_kwargs = {
            ...
            **(
                {"tactile_pixel_values": model_input["tactile_pixel_values"]}
                if "tactile_pixel_values" in model_input
                else {}
            ),
        }
        reference_action = data.get("reference_action")
        if reference_action is not None:
            inference_kwargs["reference_action"] = self._prepare_frs_reference_action(
                reference_action,
                reference_action_space=data.get("reference_action_space", "physical"),
                data=data,
                robot_state=state,
                dmtac05_model=dmtac05_model,
                action_mask=action_mask,
            )
        if torch.cuda.is_available():
        ...
        if self.backend == "fast":
            ...
            precision_context = (
                dmtac05_autocast(self.device)
                if dmtac05_model.precision_policy == FP32_MIXED_PRECISION_POLICY
                else nullcontext()
            )
            with precision_context:
                if data.get("sampling_mode") == "tac_frs":
                    runtime = getattr(self, "tac_frs_runtime", None)
                    if runtime is None:
                        raise ValueError("Checkpoint does not contain Tac-FRS weights")
                    with torch.no_grad():
                        actions, _ = runtime.generate(
                            dmtac05_model, inference_kwargs, data["tac_frs_latent"],
    ...
    def _prepare_frs_reference_action(
        self,
        reference_action,
        *,
        reference_action_space: str = "physical",
        data: dict,
        robot_state,
        dmtac05_model,
        action_mask: torch.Tensor,
    ) -> torch.Tensor:
        chunk_size = int(dmtac05_model.model.config.chunk_size)
        action_dim = int(dmtac05_model.model.config.action_dim)
        dtype = dmtac05_model.model.action_in_proj.weight.dtype
        if reference_action_space not in ("physical", "policy_normalized"):
            raise ValueError("reference_action_space must be physical or policy_normalized")
        if torch.is_tensor(reference_action):
            reference_action = reference_action.detach().float().cpu().numpy()
        arr = np.asarray(reference_action, dtype=np.float32)
        if not np.isfinite(arr).all():
            raise ValueError("reference_action must contain only finite values")
        if reference_action_space == "policy_normalized":
            return prepare_action_chunk(
                torch.as_tensor(arr),
                batch_size=1,
        ...
        if arr.ndim == 1:
            arr = np.repeat(arr[None, :], chunk_size, axis=0)
        elif arr.ndim != 2:
            raise ValueError(
                "reference_action must have shape [action_dim] or "
                f"[chunk_size, action_dim], got {arr.shape}"
            )
        if arr.shape[-1] != self.output_action_dim:
            raise ValueError(
                "reference_action last dim must match output_action_dim "
                f"{self.output_action_dim}, got {arr.shape[-1]}"
            )
        if arr.shape[0] != chunk_size:
            raise ValueError(
                "reference_action time length must match chunk_size "
                f"{chunk_size}, got {arr.shape[0]}"
            )
        ...
        if self.use_absolute_action:
            sample = self.action_relative(sample)
        sample = self.action_normalize(sample)
        return prepare_action_chunk(
            torch.as_tensor(sample["action"]),
            batch_size=1,
            chunk_size=chunk_size,
            action_dim=action_dim,
            dtype=dtype,
    ...
    def _apply_v1_sampling(self, sampling):
        if sampling is None:
            return None
        if not isinstance(sampling, dict):
            raise ValueError("sampling must be a JSON object")

        space = sampling.get("reference_action_space", "physical")
        if space not in ("physical", "policy_normalized"):
            raise ValueError("reference_action_space must be physical or policy_normalized")
        if "reference_action_space" in sampling and sampling.get("reference_action") is None:
            raise ValueError("reference_action_space requires reference_action")
        ...
        return sampling.get("reference_action")
    ...
    def _prepare_input(self, body: dict) -> dict:
        ...
        data = self._prepare_model_input(
        ...
        tactile_config = getattr(
            unwrap_dmtac05_model(self.model).config, "tactile_config", None
        )
        marker = observation.get("tactile_history")
        if tactile_config is not None:
            from tacmind0.tactile.preprocessing import prepare_history

            if not isinstance(marker, dict) or set(marker) != {"left", "right"}:
                raise ValueError(
                    "observation.tactile_history requires left/right arrays"
                )
            for side in ("left", "right"):
                if not isinstance(marker[side], list) or len(marker[side]) != 8:
                    raise ValueError(
                        "Each tactile_history side requires eight oldest-to-current images"
                    )
            frames = [
                [
                    self._decode_b64_image(
                        value, f"tactile_history.{side}", require_rgb=True
                    )
                    for value in marker[side]
                ]
                for side in ("left", "right")
            ]
            data["tactile_pixel_values"] = prepare_history(*frames).unsqueeze(0)
        elif marker is not None:
            raise ValueError("Tactile input supplied to a vision-only checkpoint")
        return data
    ...
    def _infer(self):
        try:
            ...
            reference_action = self._apply_v1_sampling(body.get("sampling"))
            t0 = time.monotonic()
            data = self._prepare_input(body)
            if reference_action is not None:
                data["reference_action"] = reference_action
                data["reference_action_space"] = body["sampling"].get(
                    "reference_action_space", "physical"
                )
            ...
            return jsonify(
                {
                    ...
                    "metadata": {
                        ...
                        "sampling_mode": "frs" if reference_action is not None else "without_frs",
                    },
                }
            )
...
@dataclass
class DMTac05Exp(Config):
    use_lora: bool | None = field(default=False)
    task: Literal["train", "inference"] = field(default="train")
    model_config: DMTac05ModelConfig = field(default_factory=DMTac05ModelConfig)
    optimizer_config: DMTac05OptimizerConfig = field(
        default_factory=DMTac05OptimizerConfig
    )
    trainer_config: DMTac05TrainerConfig = field(default_factory=DMTac05TrainerConfig)
    data_config: DMTac05DataConfig = field(default_factory=DMTac05DataConfig)
    inference_config: DMTac05InferenceConfig = field(
        default_factory=DMTac05InferenceConfig
    )
    ...
    def _initialize_train(self):
        ...
        dmtac05_model = unwrap_dmtac05_model(self.model)
        dmtac05_model.config.use_cache = False
        dmtac05_model.model.vlm.config.use_cache = False
    ...
    def train(self):
        ...
        dmtac05_model = unwrap_dmtac05_model(self.model)
        dmtac05_model.config.use_cache = True
        dmtac05_model.model.vlm.config.use_cache = True
    ...
    def _initialize_inference_runtime(self) -> None:
        ...
        if use_fast_backend or self.inference_config.prefix_seq_len_buckets:
            ...
            DMTac05FastInferRuntime.resolve_prefix_seq_len_buckets(
    ...
    def inference(self) -> None:
        ...
        if hasattr(self.inference_config, "_release_tac_frs"):
            app.add_url_rule("/v1/tac_frs/release", "tac_frs_release",
                self.inference_config._release_tac_frs, methods=["POST"])
        app.run(
            host="0.0.0.0",
