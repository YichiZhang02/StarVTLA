...
from tacmind0.constants.robot import (
    HISTORY_TOKENS_PER_IMAGE,
    ActionMode,
    RobotStateDesc,
)
from tacmind0.data.augmentations import TransformPipeline
from tacmind0.data.normalize import NormStats, NormStatsFile, load_norm_stats_file
...
class PixelTransform:
    ...
    ``dmtac05_history`` inference path (PadToSquare + Resize before image_processor).
    ...
    def __init__(
        ...
        *,
        image_pipelines: list[TransformPipeline] | None = None,
    ):
        if (transform_pipeline is None) == (image_pipelines is None):
            raise ValueError(
                "Pass exactly one transform_pipeline or image_pipelines argument"
            )
        if image_pipelines is not None and not image_pipelines:
            raise ValueError("image_pipelines must not be empty")
        self.transform_pipeline = transform_pipeline
        self.image_pipelines = image_pipelines

    def _transform_images(self, images, pipelines=None):
        if pipelines is None:
            pipeline = self.transform_pipeline
            if pipeline is None:
                pipeline = self.image_pipelines[0]
            pipelines = [pipeline] * len(images)
        elif len(pipelines) != len(images):
            raise ValueError(
                "image_pipelines must contain one pipeline per current image"
            )
        return [
            Image.fromarray(
                pipeline(image=np.array(image))["image"],
                mode="RGB",
            )
            for image, pipeline in zip(images, pipelines, strict=True)
        ]

    def __call__(self, data):
        data["images"] = self._transform_images(data["images"], self.image_pipelines)
...
class LoadImages:
    ...
    def __init__(
        self, image_keys: list[str], image_dir: str = "", *, require_rgb: bool = False
    ):
    ...
    def __call__(self, data):
        ...
        for key in self.image_keys:
            image_url = os.path.join(self.image_dir, data[key]["url"])
            image_type = data[key]["type"]
            if image_type == "image":
                images.append(_load_image(image_url, require_rgb=self.require_rgb))
...
class ChatTokenization:
    ...
    def __call__(self, data):
        ...
        return {
            ...
            **(
                {"tactile_pixel_values": data["tactile_pixel_values"]}
                if "tactile_pixel_values" in data
                else {}
            ),
        }
...
def _load_image(image_url: str, *, require_rgb: bool = False) -> Image.Image:
    with megfile.smart_open(image_url, mode="rb") as f:
        image = Image.open(io.BytesIO(f.read()))
        if require_rgb and image.mode != "RGB":
            raise ValueError("Marker image must use RGB mode with three uint8 channels")
