...
def _load_dmtac05_config_for_inference(
    model_config_path: str,
    config_overrides: dict | None = None,
) -> DMTac05Config:
    config = DMTac05Config.from_pretrained(model_config_path)
    for attr, value in (config_overrides or {}).items():
        if hasattr(config, attr):
            setattr(config, attr, value)
            logger.info("Overriding DMTac05 inference config {}={}", attr, value)
        else:
            logger.warning(
                "Ignoring unknown DMTac05 inference config override {}={}",
                attr,
                value,
            )
    return config
...
class DMTac05LoraConfig:
    ...
    modules_to_save: list[str] = field(
        default_factory=lambda: [
            ...
            "time_mlp_in",
            "time_mlp_out",
            "dmtac05_time_modulators",
        ]
    )
    dump_trainable_path: str | None = field(
        default=(
            "user_checkpoints/dmtac05_sft/trainable_summaries/"
            "dmtac05_lora_libero_pi0_all.json"
        )
    )


_DMTac05_TIME_MODULATOR_ALIASES = {
    "dmtac05_time_modulators",
    "dmtac05_action_time_modulators",
    "action_time_modulators",
}


def unwrap_dmtac05_model(model: torch.nn.Module) -> DMTac05ForConditionalGeneration:
...
def _expand_modules_to_save(
    ...
    for name in modules_to_save:
        if name not in _DMTac05_TIME_MODULATOR_ALIASES:
            append(name)
            continue
        ...
        if (
            ...
            raise ValueError(
                ...
                "DMTac05 action time modulators are not present on the model."
            )
        if len(input_modulators) != len(mlp_modulators):
            raise ValueError(
                "DMTac05 action time modulator lists have inconsistent lengths: "
...
def _resolve_target_modules(
    ...
    if not resolved:
        raise ValueError(
            "DMTac05 LoRA target_modules='all-linear' found no Linear modules."
        )

    logger.info(
        "Resolved DMTac05 LoRA target_modules='all-linear' to {} target suffixes.",
        len(resolved),
    )
...
def _patch_peft_fsdp_tied_weights(model: torch.nn.Module) -> None:
    ...
    if patched:
        logger.info("Cleared tied-weight metadata for DMTac05 LoRA/FSDP compatibility.")
...
def _cast_trainable_params_to_model_dtype(model: torch.nn.Module) -> None:
    ...
    if cast_count:
        logger.info(
            "Cast {} DMTac05 LoRA trainable parameters to model dtype {} for FSDP.",
            cast_count,
            target_dtype,
        )
...
def _dump_trainable_summary(
    ...
    logger.info(
        "DMTac05 LoRA trainable summary: style={}, trainable={} frozen={} ratio={:.6f}",
        lora_config.style,
        trainable_total,
        frozen,
    ...
    if lora_config.dump_trainable_path:
        ...
        logger.info(
            "Wrote DMTac05 LoRA trainable summary to {}",
            lora_config.dump_trainable_path,
        )


def apply_lora_to_dmtac05_model(
    model: DMTac05ForConditionalGeneration,
    lora_config: DMTac05LoraConfig,
    ...
    try:
        ...
        raise ImportError(
            "DMTac05 LoRA requires `peft` to be installed in the training environment"
        ) from exc
    ...
    if modules_to_save != lora_config.modules_to_save:
        logger.info(
            "Expanded DMTac05 LoRA modules_to_save: raw={} expanded_count={}",
    ...
    logger.info(
        "Enabled DMTac05 LoRA: style={}, r={}, alpha={}, target_modules={}, "
        "modules_to_save={}",
        lora_config.style,
        lora_config.r,
...
def load_dmtac05_model_for_inference(
    ...
    *,
    ...
    if not _is_lora_checkpoint(model_name_or_path):
        config = _load_dmtac05_config_for_inference(model_name_or_path, config_overrides)
    ...
    try:
        ...
        raise ImportError("Loading a LoRA DMTac05 checkpoint requires `peft`") from exc
    ...
    logger.info("Loading DMTac05 LoRA base model from {}", base_model_path)
    config_source = (
        model_name_or_path
        if os.path.exists(os.path.join(model_name_or_path, "config.json"))
        else base_model_path
    )
    config = _load_dmtac05_config_for_inference(config_source, config_overrides)
    base_model = DMTac05ForConditionalGeneration.from_pretrained(
        base_model_path,
        config=config,
        dtype=dtype,
        trust_remote_code=trust_remote_code,
    )
    logger.info("Loading DMTac05 LoRA adapter from {}", model_name_or_path)
    model = PeftModel.from_pretrained(base_model, model_name_or_path)
    logger.info("Merging DMTac05 LoRA adapter into the base model for inference")
