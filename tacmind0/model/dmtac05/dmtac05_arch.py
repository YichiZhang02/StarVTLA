"""DMTac05 model architecture built on a Gemma3 VLM and Action Expert."""
...
from tacmind0.constants.robot import HISTORY_POOL_SIZE
from tacmind0.model.base import (
    DMBaseConfig,
    DMPreTrainedModel,
)
...
class DMTac05Config(DMBaseConfig):
    """DMTac05 model configuration."""
...
class DMTac05ActionExpert(Gemma3TextModel):
    """Gemma3 action expert with DMTac05 suffix-only forward.
    ...
    def __init__(
        ...
        # Transformers 5.5 moved the layer type from the decoder layer to
        # ``self_attn.layer_type``. DMTac05's suffix/cache code uses the older
        # ``decoder_layer.attention_type`` spelling, so expose a compatibility
        # attribute once at construction time.
        for layer in self.layers:
            if not hasattr(layer, "attention_type"):
                layer.attention_type = getattr(
    ...
    def forward(
        ...
        *,
        ...
        tactile_modulations: torch.Tensor | None = None,
        **kwargs,
    ) -> torch.Tensor:
        if suffix_embeds is None:
            raise RuntimeError(
                "DMTac05ActionExpert only supports suffix forward. "
                "Pass suffix_embeds, position_ids, prefix_cache_keys, and "
                "prefix_cache_values."
            )
        if position_ids is None:
            raise ValueError(
                "position_ids is required for DMTac05ActionExpert suffix forward."
            )
        if prefix_cache_keys is None or prefix_cache_values is None:
            raise ValueError(
                "prefix_cache_keys and prefix_cache_values are required for "
                "DMTac05ActionExpert suffix forward."
            )
        if adarms_cond is None:
            raise ValueError(
                "adarms_cond is required for DMTac05ActionExpert suffix forward."
            )
        return self._suffix_forward_layers(
            ...
            tactile_modulations=tactile_modulations,
        )
    ...
    def _suffix_forward_layers(
        ...
        # These values depend on positions/mask, not on hidden states. Keep
        # reuse local to this call so requests and autograd graphs stay isolated.
        rotary_cache = {}
        if (
            self._suffix_attn_backend == "sdpa"
        ...
        for layer_idx in range(num_layers):
            ...
            if self.training and self.gradient_checkpointing:
                hidden_states = torch.utils.checkpoint.checkpoint(
                    ...
                    layer_cache_keys,
                    layer_cache_values,
                    None if tactile_modulations is None else tactile_modulations[layer_idx],
                    rotary_cache,
                    use_reentrant=False,
                )
            else:
    ...
    def _compute_suffix_layer(
        ...
        tactile_modulation: torch.Tensor | None = None,
        ...
        if tactile_modulation is not None:
            scale_a, shift_a, scale_m, shift_m = tactile_modulation.chunk(4, dim=-1)
            prenorm = (prenorm.float() * (1 + scale_a) + shift_a).to(prenorm.dtype)
        ...
        rotary_key = (layer_type, query_states.dtype)
        if rotary_cache is None:
            cos, sin = self.rotary_emb(query_states, position_ids, layer_type=layer_type)
        else:
            if rotary_key not in rotary_cache:
                rotary_cache[rotary_key] = self.rotary_emb(
                    query_states, position_ids, layer_type=layer_type
                )
            cos, sin = rotary_cache[rotary_key]
        ...
        if tactile_modulation is not None:
            mlp_input = (mlp_input.float() * (1 + scale_m) + shift_m).to(mlp_input.dtype)
...
@dataclass
class DMTac05OutputWithPast(Gemma3CausalLMOutputWithPast):
    fm_loss: torch.FloatTensor | None = None


@dataclass
class DMTac05SuffixGraphProfile:
...
class DMTac05Model(DMPreTrainedModel):
    """Core DMTac05 model.
    ...
    def __init__(self, config: DMTac05Config):
        super().__init__(config)

        vlm_config_ref = config.vlm_config
        action_config_ref = config.action_config

        logger.info(f"Final DMTac05Config: {config}")
        ...
        self.action_expert = DMTac05ActionExpert(action_config_ref)
...
class DMTac05ForConditionalGeneration(DMPreTrainedModel):
    """Full DMTac05 model with training and action inference support.

    Methods:
    - ``forward()``: training with flow matching.
    - ``inference_action()``: Euler sampling; optional Flow Reversal Steering
      when ``reference_action`` is provided.
    """

    config_class = DMTac05Config
    ...
    def __init__(self, config: DMTac05Config):
        ...
        saved_action_backend = getattr(config, "tac_frs_action_attention_backend", None)
        if saved_action_backend is not None:
            self.model.action_expert.set_action_attention_backend(saved_action_backend)

    @classmethod
    def from_pretrained(cls, *args, **kwargs):
        result = super().from_pretrained(*args, **kwargs)
        model = result[0] if isinstance(result, tuple) else result
        if getattr(model.config, "tactile_config", None) is not None:
            model.tactile_encoder.rebuild_position_buffers()
            model.tactile_encoder.requires_grad_(False)
    ...
    def _real_init(self, config: DMTac05Config):
        self.model = DMTac05Model(config)
        if config.tactile_config is not None:
            from tacmind0.tactile.encoder import FrozenTactileEncoder, TactileFiLM

            self.tactile_encoder = FrozenTactileEncoder(
                config.tactile_config["backbone"]
            )
            self.tactile_film = TactileFiLM(config.vlm_config.text_config.hidden_size)
        self.post_init()
        if config.tactile_config is not None:
            self.tactile_film.reset_identity()
    ...
    def set_attention_implementation(
        ...
        *,
        ...
        assert llm_attn_valid and vision_attn_valid and action_attn_valid, (
            "Invalid DMTac05 attention implementation: "
    ...
    def inference_action(
        ...
        **kwargs,
        ...
        if "reference_action" in kwargs:
            reference_action = kwargs.pop("reference_action")
    ...
    def _inference_action_impl(
        ...
        **kwargs,
        ...
        if reference_action is None:
            ...
            x_t = self._integrate_action_flow(
                x_t,
                input_ids=input_ids,
                kv_cache=kv_cache,
                prefix_len=prefix_len,
        ...
        return self._integrate_action_flow(
            x_t,
            input_ids=input_ids,
            kv_cache=kv_cache,
            prefix_len=prefix_len,
    ...
    def _integrate_action_flow(
        ...
        *,
        ...
        device = x_t.device
        dtype = self.model.action_in_proj.weight.dtype
        time_val, dt = flow_euler_schedule(diffusion_steps, reverse=reverse)
        ...
        for _ in range(int(diffusion_steps)):
            ...
            suffix_call = self._suffix_forward
            if checkpoint_steps and torch.is_grad_enabled():
                from functools import partial

                from torch.utils.checkpoint import checkpoint
                suffix_call = partial(checkpoint, self._suffix_forward, use_reentrant=False)
            suffix_out = suffix_call(
                suffix_embeds=suffix_embeds,
                attention_mask=suffix_attn_mask,
                position_ids=suffix_position_ids,
                past_key_values=kv_cache,
    ...
    def _can_use_suffix_graph(self, initial_noise: torch.Tensor) -> bool:
        return (
            not self.training
            and getattr(self.config, "use_suffix_graph", True)
    ...
    def _cache_suffix_graph_profile(
        self,
        profile_key: tuple,
        profile: DMTac05SuffixGraphProfile,
    ) -> None:
    ...
    def _disable_suffix_graph_profile(
        ...
        logger.warning(
            "Disabling DMTac05 suffix CUDA Graph profile after failure; "
    ...
    def _run_suffix_graph_step(self, profile: DMTac05SuffixGraphProfile) -> None:
        x_t = profile.state
        if profile.action_mask is not None:
    ...
    def _compute_prefix_cache(
        ...
        *,
        ...
        if self.config.tactile_config is not None:
            if tactile_pixel_values is None:
                raise ValueError("TacDream requires tactile_pixel_values")
            if pixel_values is None:
                raise ValueError("TacDream requires current vision pixels")
            z = self.tactile_encoder(tactile_pixel_values)
            if z.shape[0] != input_ids.shape[0]:
                raise ValueError("Tactile batch does not match language batch")
            features = vlm_model.get_image_features(
                pixel_values.to(dtype=next(vlm_model.vision_tower.parameters()).dtype),
                return_dict=True,
            ).pooler_output
            features = self.tactile_film(features, z)
            if prefix_inputs_embeds is None:
                prefix_inputs_embeds = embed_tokens(input_ids)
            image_mask = input_ids == vlm_model.config.image_token_id
            expected = 2 * features.shape[1]
            if not torch.all(image_mask.sum(dim=1) == expected):
                raise ValueError(
                    "Image token count must match two current views per sample"
                )
            prefix_inputs_embeds = prefix_inputs_embeds.masked_scatter(
                image_mask.unsqueeze(-1).expand_as(prefix_inputs_embeds),
                features.to(prefix_inputs_embeds.dtype),
            )
            pixel_values = None
        elif tactile_pixel_values is not None:
...
AutoConfig.register("dmtac05", DMTac05Config)
AutoModelForCausalLM.register(DMTac05Config, DMTac05ForConditionalGeneration)
