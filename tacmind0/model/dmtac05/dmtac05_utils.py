"""DMTac05 utility functions for cache handling, time embeddings, and masks."""
...
from tacmind0.constants.precision import COMPUTE_DTYPE, TF32_ENABLED


def dmtac05_autocast(device: torch.device | str):
    """Run eligible DMTac05 operators in BF16 on the given device."""
...
def flow_euler_schedule(
    num_steps: int, *, reverse: bool = False
) -> tuple[float, float]:
    """DMTac05 flow time: ``t=0`` is action, ``t=1`` is noise.

    ``reverse=False`` denoises (noise → action). ``reverse=True`` noises
    (action → noise) for Flow Reversal Steering.
    """
    steps = int(num_steps)
    if steps <= 0:
        raise ValueError(f"num_steps must be positive, got {num_steps}")
    dt = (1.0 if reverse else -1.0) / steps
    return (0.0 if reverse else 1.0), dt
...
def prepare_action_chunk(
    ...
    *,
    ...
    """Broadcast a reference action to ``[B, chunk_size, action_dim]``.

    A 1-D vector is tiled across the chunk (coarse directional command).
    """
    x = torch.as_tensor(action, device=device, dtype=dtype)
    batch_size = int(batch_size)
    chunk_size = int(chunk_size)
    action_dim = int(action_dim)
    if x.ndim == 1:
        x = x.view(1, 1, -1).expand(batch_size, chunk_size, -1).contiguous()
    elif x.ndim == 2:
        if x.shape[0] == 1:
            x = x.expand(chunk_size, -1)
        if x.shape[0] != chunk_size:
            raise ValueError(
                "reference_action has time length "
                f"{int(x.shape[0])}, expected chunk_size={chunk_size}"
            )
        x = x.unsqueeze(0).expand(batch_size, -1, -1).contiguous()
    elif x.ndim == 3:
        if x.shape[0] == 1 and batch_size > 1:
            x = x.expand(batch_size, -1, -1).contiguous()
        if x.shape[0] != batch_size or x.shape[1] != chunk_size:
            raise ValueError(
                "reference_action has shape "
                f"{tuple(x.shape)}, expected ({batch_size}, {chunk_size}, action_dim)"
            )
    else:
        raise ValueError(f"reference_action must have 1, 2, or 3 dims, got {x.ndim}")

    last_dim = int(x.shape[-1])
    if last_dim > action_dim:
        raise ValueError(
            f"reference_action dim {last_dim} exceeds action_dim={action_dim}"
        )
    if last_dim < action_dim:
        x = F.pad(x, (0, action_dim - last_dim))
    if action_mask is not None:
        x = x * action_mask.to(device=x.device, dtype=x.dtype)
    return x
...
def validate_action_config_compatible(action_config, language_config) -> None:
    ...
    if mismatches:
        raise ValueError(
            "DMTac05 action_config must match VLM language config for suffix KV reuse; "
            + "; ".join(mismatches)
        )
...
class SafeCacheDecoderLayer(GradientCheckpointingLayer):
    ...
    gradient checkpointing is enabled. DMTac05 prefix forward still needs to write
...
class OverwriteDynamicLayer(CacheLayerMixin):
    ...
    def get_mask_sizes(self, cache_position: torch.Tensor | int) -> tuple[int, int]:
        kv_offset = 0
        # Transformers 5.12 passes a query length; older versions pass positions.
        query_length = (
            cache_position
