"""Service wrapper for bucketed DMTac05 fast inference.
...
from tacmind0.constants.precision import FP32_MIXED_PRECISION_POLICY
from tacmind0.constants.robot import HISTORY_TOKENS_PER_IMAGE
from tacmind0.infer.dmtac05_infer_arch import DMTac05FastForCausalLM
from tacmind0.infer.dmtac05_trt_utils import (
    MAX_HISTORY_IMAGES,
    DMTac05VisionTensorRTRunner,
    pack_current_and_history_pixels,
    pool_image_features_to_history,
)
from tacmind0.model.dmtac05.dmtac05_arch import DMTac05ForConditionalGeneration
...
@dataclass
class DMTac05StaticGraphProfile:
    bucket_len: int
    prefix_cache: Cache
    prefix_buffers: Any
    suffix_buffers: Any
...
class DMTac05FastInferRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.model = DMTac05FastForCausalLM(model)
        self.device = next(model.parameters()).device
        self.dmtac05 = self.model.dmtac05
        ...
        self.graph_profiles: dict[int, DMTac05StaticGraphProfile] = {}
        ...
        self.vision_trt_runner = DMTac05VisionTensorRTRunner(
        ...
        tokens_per_image = int(self.dmtac05.vlm.model.config.mm_tokens_per_image)
    ...
    def _precision_context(self):
        if self.model.precision_policy == FP32_MIXED_PRECISION_POLICY:
            return dmtac05_autocast(self.device)
        return nullcontext()

    def _make_static_prefix_cache(self) -> Cache:
        cache_config = self.dmtac05.language_model.config
    ...
    def _initialize_graph_profiles(self) -> None:
        ...
        logger.info(
            "Pre-captured {} DMTac05 prefix+suffix graph profiles for buckets {}",
    ...
    def _make_graph_profile(
        ...
        *,
        ...
        return DMTac05StaticGraphProfile(
            bucket_len=bucket_len,
    ...
    def _run_graph_profile_ops(self, profile: DMTac05StaticGraphProfile) -> None:
    ...
    def _capture_graph_profile(self, profile: DMTac05StaticGraphProfile) -> None:
    ...
    def _forward_locked(
        ...
        *,
        ...
        # FRS needs a reverse ODE pass; the captured graph is forward-only.
        if not use_frs:
    ...
    def _select_graph_profile(self, request_len: int) -> DMTac05StaticGraphProfile | None:
    ...
    def _replay_graph_profile(
        self,
        profile: DMTac05StaticGraphProfile,
        *,
        images: torch.Tensor,
        input_ids: torch.Tensor,
        attention_mask: torch.Tensor,
    ...
    def _suffix_decode(
        ...
        *,
        ...
        state = diffusion_input_noise
        if use_frs:
            state = self.model.decode(
                **decode_kwargs,
                diffusion_input_noise=state,
                reverse=True,
            )
    ...
    def inference_action(
        ...
        **kwargs,
        ...
        if reference_action is None:
            ...
            diffusion_input_noise = prepare_action_chunk(
                reference_action,
                batch_size=int(input_ids.shape[0]),
                chunk_size=int(self.suffix_len),
