#!/usr/bin/env python3
"""TensorRT build and runtime helpers for the DMTac05 fast path.

This module centralizes engine loading, ONNX-to-engine export, execution
context setup, tensor/binding utilities, and the DMTac05 vision TensorRT runner
...
DEFAULT_DMTac05_VISION_TRT_ENGINE_PATH = "checkpoints/trt_engines/dmtac05_vision.engine"
...
def resolve_dmtac05_vision_trt_engine_path(
    engine_path: str | Path | None,
) -> Path:
    """Resolve the DMTac05 fast-path vision TensorRT engine path."""
    if engine_path:
        return Path(engine_path).expanduser()

    resolved_path = Path(DEFAULT_DMTac05_VISION_TRT_ENGINE_PATH).expanduser()
    logger.info(
...
def ensure_dmtac05_fast_inference_engine(
    *,
    ...
    """Ensure the DMTac05 fast backend vision TensorRT engine is ready."""
    if not checkpoint:
    ...
    resolved_engine_path = resolve_dmtac05_vision_trt_engine_path(engine_path)
    return ensure_dmtac05_vision_trt_engine(
...
def ensure_dmtac05_vision_trt_engine(
    *,
    checkpoint: str | Path,
    ...
    """Ensure the static-shape DMTac05 vision TensorRT engine is ready."""
    if num_images <= 0:
    ...
    command = [
        sys.executable,
        "-m",
        "dmtac05.infer.build_vision_trt",
        "--checkpoint",
        str(checkpoint_path),
        "--onnx-path",
        str(onnx_path),
    ...
    logger.info(
        "Ensuring DMTac05 fast vision TensorRT engine via: {}",
        shlex.join(command),
    )
    try:
        ...
        raise RuntimeError(
            f"Failed to prepare DMTac05 vision TensorRT engine: {engine_path}"
        ) from exc
...
class _TensorRTRunnerBase:
    """Shared TensorRT engine/context setup for fixed DMTac05 runners."""
...
class DMTac05VisionTensorRTRunner(_TensorRTRunnerBase):
    """Run a static-shape TensorRT engine that returns DMTac05 image features."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            engine_path,
            device=device,
            runtime_context="DMTac05 vision TensorRT inference",
            engine_context="DMTac05 vision",
        )
        if len(self.input_names) != 1 or len(self.output_names) != 1:
            raise RuntimeError(
                "DMTac05 vision TensorRT engine must have exactly one input and one "
        ...
        if len(self.input_shape) != 4 or any(dim <= 0 for dim in self.input_shape):
            raise RuntimeError(
                "DMTac05 vision TensorRT engine must have a static 4D input "
    ...
    def _set_dynamic_input_shape(self, shape: tuple[int, ...]) -> tuple[int, ...]:
        ...
        if any(dim < 0 for dim in output_shape):
            raise RuntimeError(
                "DMTac05 vision TensorRT output shape is still dynamic after setting "
    ...
    def __call__(
        ...
        *,
        ...
        input_tensor = prepare_engine_input_tensor(
            ...
            label="DMTac05 vision TensorRT input",
        )
        input_shape = tuple(int(dim) for dim in input_tensor.shape)
        if len(input_shape) != 4:
            raise ValueError(
                "DMTac05 vision TensorRT input must be a 4D image tensor, "
                f"got shape {input_shape}."
            )
        if int(input_shape[0]) != self.num_images:
            raise ValueError(
                "DMTac05 vision TensorRT image count mismatch: engine was "
        ...
        if not any(dim < 0 for dim in expected_shape) and input_shape != expected_shape:
            raise ValueError(
                "DMTac05 vision TensorRT input shape mismatch: "
        ...
        if output_tensor is None:
            ...
            raise ValueError(
                "DMTac05 vision TensorRT output tensor does not match the engine: "
        ...
        if not ok:
            raise RuntimeError("DMTac05 vision TensorRT execution failed.")
