#!/usr/bin/env python3
"""Export and build the DMTac05 vision TensorRT engine for fast inference.

This script loads a DMTac05 checkpoint, exports the vision tower plus multimodal
...
from tacmind0.model.dmtac05.dmtac05_lora import load_dmtac05_model_for_inference


class DMTac05VisionFeatureModule(nn.Module):
...
def _load_model(checkpoint: Path) -> DMTac05ForConditionalGeneration:
    model = load_dmtac05_model_for_inference(
        str(checkpoint),
        dtype=torch.float16,
        trust_remote_code=True,
    )
    if not isinstance(model, DMTac05ForConditionalGeneration):
        raise TypeError(
            "build_vision_trt.py expects a DMTac05ForConditionalGeneration-compatible "
...
def export_vision_onnx(
    *,
    ...
    wrapper = DMTac05VisionFeatureModule(vlm_model).eval().to("cuda")
    dummy = torch.randn(
        num_images,
        3,
        image_size,
...
def _vision_engine_num_images(engine_path: Path) -> int | None:
    if not engine_path.is_file() or engine_path.stat().st_size <= 0:
        return None
    trt = load_tensorrt("Inspecting DMTac05 vision TensorRT engine")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="DMTac05 checkpoint used to export/build the vision TensorRT engine.",
    )
    parser.add_argument(
        "--onnx-path",
        type=Path,
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Building the DMTac05 vision TensorRT engine requires CUDA.")
    ...
    print(
        "Built DMTac05 vision TensorRT engine: "
