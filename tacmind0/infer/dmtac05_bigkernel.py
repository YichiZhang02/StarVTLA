"""Shape-specialized Triton kernels and buffers for DMTac05 fast inference.

This module implements the fixed-shape GPU kernels used by the DMTac05 fast path,
...
SUPPORTED_SUFFIX_SHAPE = DMTac05TransformerShape(
...
@dataclass
class DMTac05PrefixDecoderBuffers:
    rows: int
    batch_size: int
    seq_len: int
    dtype: torch.dtype
...
def _suffix_launch_config(buffers: DMTac05BigKernelBuffers) -> KernelLaunchConfig:
...
def make_suffix_buffers(
    *,
    ...
    dtype: torch.dtype,
    device: torch.device,
    shape: DMTac05TransformerShape = SUPPORTED_SUFFIX_SHAPE,
) -> DMTac05BigKernelBuffers:
    ...
    return DMTac05BigKernelBuffers(
        rows=rows,
...
def make_prefix_decoder_buffers(
    *,
    ...
    device: torch.device,
    shape: DMTac05TransformerShape,
) -> DMTac05PrefixDecoderBuffers:
    rows = int(batch_size) * int(seq_len)

    def empty(shape: tuple[int, ...]) -> torch.Tensor:
        return torch.empty(shape, device=device, dtype=dtype)

    return DMTac05PrefixDecoderBuffers(
        rows=rows,
        batch_size=int(batch_size),
        seq_len=int(seq_len),
        dtype=dtype,
...
def project_qkv_and_launch_qknorm_rope_split(
    ...
    buffers: DMTac05BigKernelBuffers | DMTac05PrefixDecoderBuffers,
...
def launch_input_adarmsnorm(
    ...
    buffers: DMTac05BigKernelBuffers,
    *,
    eps: float,
) -> None:
    shape = buffers.shape
...
def launch_prefix_attn_postnorm_residual_prenorm(
    attn_embeds: torch.Tensor,
    residual: torch.Tensor,
    buffers: DMTac05PrefixDecoderBuffers,
