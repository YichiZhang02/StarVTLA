"""Fast DMTac05 model-side prefill and suffix decode helpers.

This module adapts ``DMTac05ForConditionalGeneration`` into an inference-only
...
try:
    import tacmind0.infer.dmtac05_bigkernel as bigkernel
...
def _prefix_decoder_shape(layer) -> "bigkernel.DMTac05TransformerShape":
    attn = layer.self_attn
    head_dim = int(attn.head_dim)
    return bigkernel.DMTac05TransformerShape(
...
def _supported_prefix_decoder_shape() -> "bigkernel.DMTac05TransformerShape":
    return bigkernel.DMTac05TransformerShape(
...
class DMTac05FastActionExpert(DMTac05ActionExpert):
    """Inference-only action expert extension with Triton suffix big-kernel."""

    @classmethod
    def from_action_expert(
        cls,
        action_expert: DMTac05ActionExpert,
    ) -> "DMTac05FastActionExpert":
...
class DMTac05FastForCausalLM:
    ...
    def __init__(
        ...
        self.model = model
        self.dmtac05 = model.model
        self.precision_policy = getattr(
            model,
            "precision_policy",
            BF16_MIXED_PRECISION_POLICY,
        )
        self.action_expert = DMTac05FastActionExpert.from_action_expert(
            self.dmtac05.action_expert
        )
        self.padding_idx = int(self.dmtac05.vlm.model.language_model.padding_idx)
        # Maximum processed multimodal prefix length supported by FastInfer.
        self.prefix_len = 1024
        self.suffix_len = int(self.dmtac05.config.chunk_size)
        self.action_dim = int(self.dmtac05.config.action_dim)
        self.noise_dtype = self.dmtac05.action_in_proj.weight.dtype
        ...
        self.suffix_time_modulation_cache: dict[tuple, tuple] = {}
        self.action_expert.setup_fast_bigkernel_suffix()

    def setup_fast_prefix_decoder(self) -> None:
        language_model = self.dmtac05.vlm.model.language_model
    ...
    def _action_input_proj(self, x: torch.Tensor) -> torch.Tensor:
        return linear_fp32(x, self.dmtac05.action_in_proj).to(self.suffix_kernel_dtype)

    def _action_output_proj(self, x: torch.Tensor) -> torch.Tensor:
        return linear_fp32(x, self.dmtac05.action_out_proj)
    ...
    def _merge_image_features_into_inputs_embeds(
        ...
        *,
        ...
        image_token_id = int(self.dmtac05.vlm.model.config.image_token_id)
        ...
        *,
        ...
        if history_features is None:
            if history_pixel_values is None or int(history_pixel_values.shape[0]) == 0:
                return inputs_embeds
            vlm_model = self.dmtac05.vlm.model
    ...
    def _build_suffix_time_modulations(
        ...
        *,
        ...
        time0, dt = flow_euler_schedule(diffusion_steps, reverse=reverse)
        time_values = torch.tensor(
            [time0 + step_idx * dt for step_idx in range(int(diffusion_steps))],
            device=device,
            dtype=dtype,
        )
    ...
    def _get_suffix_time_modulations(
        ...
        *,
        ...
        signature = (int(batch_size), int(diffusion_steps), dtype, device, bool(reverse))
        time_modulations = self.suffix_time_modulation_cache.get(signature)
        if time_modulations is None:
            ...
            self.suffix_time_modulation_cache[signature] = time_modulations
    ...
    def decode_fast_prepared(
        ...
        *,
        ...
        x_t = x_t * action_mask
        _, dt = flow_euler_schedule(diffusion_steps, reverse=reverse)
    ...
    def _build_adarms_cond(
        ...
        ae_hidden = self.dmtac05.action_in_proj.out_features
        if self.precision_policy == FP32_MIXED_PRECISION_POLICY:
            time_emb = posemb_sincos(time, ae_hidden, max_period=4.0).to(MODEL_DTYPE)
            cond = linear_fp32(time_emb, self.dmtac05.time_mlp_in)
            cond = F.silu(cond)
            cond = linear_fp32(cond, self.dmtac05.time_mlp_out)
            return F.silu(cond)

        dtype = self.dmtac05.time_mlp_in.weight.dtype if dtype is None else dtype
        time_emb = posemb_sincos(time, ae_hidden, max_period=4.0).to(dtype)
        cond = self.dmtac05.time_mlp_in(time_emb)
        cond = F.silu(cond)
        cond = self.dmtac05.time_mlp_out(cond)
        return F.silu(cond)
